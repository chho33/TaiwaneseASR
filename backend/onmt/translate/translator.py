...
from onmt.utils.misc import tile, use_gpu
...
def load_lm(opt):
    from onmt.lm import model
    if opt.lm == None:
        return None
    lm = model.RNNModel(opt.lm_type, opt.ntoken, opt.ninp,
                        opt.nhid, opt.nlayers, tie_weights=opt.tied)
    with open(opt.lm, 'rb') as f:
...
def build_translator(opt, report_score=True, logger=None, out_file=None):
    ...
    lm = None
    if model_opt.ctc_ratio < 1:
        lm = load_lm(opt)

    translator = Translator(model, lm, fields, opt, model_opt,
