...
def build_trainer(opt, device_id, model, fields,
    ...
    train_loss = onmt.utils.loss.build_loss_compute(
        ...
        ctc_vocab=fields["tgt_feat_0"].vocab if "tgt_feat_0" in fields else None)
...
class Trainer(object):
    ...
    def validate(self, valid_iter):
        ...
        for batch in valid_iter:
            ...
            outputs, attns, enc_outs = self.model(src, tgt, src_lengths)

            # Compute loss.
            batch_stats = self.valid_loss.monolithic_compute_loss(
                batch, outputs, enc_outs, attns)
    ...
    def _gradient_accumulation(self, true_batchs, normalization, total_stats,
        ...
        for batch in true_batchs:
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                if self.grad_accum_count == 1:
                    self.model.zero_grad()
                outputs, attns, enc_outs = \
                    self.model(src, tgt, src_lengths)

                # 3. Compute loss in shards for memory efficiency.
                batch_stats = self.train_loss.sharded_compute_loss(
                    batch, outputs, enc_outs, attns, j,
                    trunc_size, self.shard_size, normalization, self.optim)
