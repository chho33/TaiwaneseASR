...
def model_opts(parser):
    ...
    group.add('--ctc_out_layer', '-ctc_out_layer', type=int, default=-1,
              help="""Output states of which layer to be passed to ctc module.
                      default to the last layer (-1).""")
    ...
    group.add('--global_attention', '-global_attention',
              type=str, default='general', choices=['dot', 'general', 'mlp', 'loc'],
              help="""The attention type to use:
                       dotprod or general (Luong) or
                       MLP (Bahdanau) or location-based.""")
    group.add('--loc_att_width', '-loc_att_width',
              type=int, default=100,
              help="""kernel size (=2*loc_att_width+1) of convolution
                      for location-based attention.""")
    group.add('--loc_att_nchan', '-loc_att_nchan',
              type=int, default=10,
              help="""number of output channels of convolution
                      for location-based attention.""")
    ...
    group.add('--ctc_ratio', '-ctc_ratio', type=float, default=0,
              help='ctc loss ratio.')
...
def preprocess_opts(parser):
    ...
    group.add('--audio_feat_type', '-audio_feat_type',default='mel',
              choices=['mel', 'mfcc'], help="Which audio feature to use. mel or mfcc")
    ...
    group.add('--input_size', '-input_size', type=int, default=240,
              help="Sample rate.")
...
def translate_opts(parser):
    ...
    # Options relevant to language model
    group.add('--lm', '-lm', type=str,
              help='Path to language model .pt file(s). '
              'language model rescoring (shallow fusion).')
    group.add('--lm_type', '-lm_type', type=str,
              help='RNN type.')
    group.add('--lbda', '-lbda', type=float, default=0,
              help='language model rescoring weight.')
    group.add('--ntoken', '-ntoken', type=int, default=3689,
              help='vocab size for language model.')
    group.add('--ninp', '-ninp', type=int, default=256,
              help='input size (embedding size) for language model.')
    group.add('--nhid', '-nhid', type=int, default=1024,
              help='input size (embedding size) for language model.')
    group.add('--nlayers', '-nlayers', type=int, default=2,
              help='input size (embedding size) for language model.')
    group.add('--tied', '-tied', action='store_true',
              help='whether to tie the input and output embeddings of language model.')
