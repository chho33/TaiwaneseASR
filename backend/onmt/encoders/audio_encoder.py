...
class AudioEncoder(nn.Module):
    ...
    def __init__(self, rnn_type, enc_layers, dec_layers, brnn, input_size,
        ...
        if ctc_ratio > 0:
            ...
            self.ctc_gen = nn.Sequential(
                nn.Linear(enc_rnn_size, tgt_vocab_size),
                gen_func
            )
    ...
    def forward(self, src, lengths=None):
        ...
        for l in range(self.enc_layers):
            ...
            memory_bank = memory_bank.transpose(0, 2)
            src = memory_bank
            if l == self.ctc_out_layer:
                enc_outs = memory_bank.contiguous()
        ...
        if self.rnn_type == 'LSTM':
            ...
            encoder_final = state
        return encoder_final, memory_bank, orig_lengths.new_tensor(lengths), enc_outs
