...
class AudioDataset(DatasetBase):
    ...
    @staticmethod
    def make_audio_examples_nfeats_tpl(path, audio_dir, audio_feat_type,
                                       sample_rate, window_size,
                                       window_stride, window,
                                       normalize_audio, truncate=None, data_iter=None):
        """
        Args:
        ...
        examples_iter = AudioDataset.read_audio_file(
            path, audio_dir, "src", audio_feat_type, sample_rate,
            window_size, window_stride, window,
            normalize_audio, truncate, data_iter=data_iter)
        num_feats = 0  # Source side(audio) has no features.

        return (examples_iter, num_feats)

    @staticmethod
    def extract_features(audio_path, audio_feat_type, sample_rate, truncate, window_size,
                         window_stride, window, normalize_audio, sound=None, sample_rate_=16000):
        global librosa, np
        import librosa
        import numpy as np

        if sound is None:
            sound, sample_rate_ = librosa.load(audio_path, sr=None)
        ...
        if len(sound.shape) > 1:
            if sound.shape[0] == 1:
                ...
                sound = sound.mean(axis=0)  # average multiple channels

        if truncate and truncate > 0:
            if sound.shape[1] > truncate:
        ...
        if audio_feat_type == 'mfcc':
            n_hid = n_mfcc = 13
            specs = librosa.feature.mfcc(sound, sample_rate, n_mfcc=n_mfcc, hop_length=hop_length, n_fft=n_fft)
            specs[0] = librosa.feature.rmse(sound, hop_length=hop_length)
        elif audio_feat_type == 'mel':
            specs = librosa.feature.melspectrogram(sound, sample_rate, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels)
        else:
            raise NotImplementedError

        floor = 1.0e-10
        if normalize_audio:
            mean = np.mean(specs, axis=-1).reshape(n_hid, 1)
            std = np.std(specs, axis=-1).reshape(n_hid, 1)
            std[std < floor] = floor
            specs = np.subtract(specs, mean)
            specs = np.divide(specs, std)
        deltas = librosa.feature.delta(specs)
        delta2s = librosa.feature.delta(specs, order=2)
        feature = np.vstack([specs, deltas, delta2s])
        feature = torch.FloatTensor(feature)
        return feature

    @staticmethod
    def read_audio_file(path, src_dir, side, audio_feat_type, sample_rate, window_size,
                        window_stride, window, normalize_audio,
                        truncate=None, data_iter=None):
        """
        Args:
            path (str): location of a src file containing audio paths.
        ...
        def get_feat(audio_path, line, sound, index):
        ...
        if path is not None:
            with codecs.open(path, "r", "utf-8") as corpus_file:
                for index, line in enumerate(tqdm(corpus_file)):
                    ...
                    example_dict = get_feat(audio_path, line, None, index)
                    yield example_dict

        elif data_iter is not None:
            for index, sound in enumerate(tqdm(data_iter)):
                example_dict = get_feat(None, str(index), sound, index)
                yield example_dict
...
class ShardedAudioCorpusIterator(object):
    ...
    """
    def __init__(self, src_dir, corpus_path, truncate, audio_feat_type, side, shard_size,
