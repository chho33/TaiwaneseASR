...
def make_features(batch, side, data_type='text'):
    ...
    if data_type == 'text':
        if len(set([level.size() for level in levels])) > 1:
            return levels[0]
...
def build_dataset(fields, data_type, src_data_iter=None, src_path=None,
                  ...
                  dynamic_dict=True, audio_feat_type='mel', sample_rate=0,
    ...
    def _make_examples_nfeats_tpl(data_type, src_data_iter, src_path, src_dir,
                                  src_seq_length_trunc, audio_feat_type, sample_rate,
        ...
        if data_type == 'text':
            ...
            src_examples_iter, num_src_feats = \
                AudioDataset.make_audio_examples_nfeats_tpl(
                    src_path, src_dir, audio_feat_type, sample_rate,
                    window_size, window_stride, window,
                    normalize_audio, data_iter=src_data_iter)
...
class DatasetLazyIter(object):
    ...
    def preprocess(self, batch):
        ...
        eos_idx = vocab.stoi[EOS_WORD]
        pad_idx = vocab.stoi[PAD_WORD]
        tgt = batch.tgt
        tgt[tgt==eos_idx] = pad_idx
        non_eos = tgt != pad_idx
        longest_non_padding = non_eos.sum(dim=0).max()
        tgt = tgt[:(longest_non_padding+1)]
        non_eos = tgt != pad_idx
        non_pad_eos = \
                torch.cat((non_eos.narrow(0,0,1), non_eos[:-1]), dim=0)
        eos = non_pad_eos - non_eos
        full_eos = tgt.new_full(tgt.size(), eos_idx)
        tgt = torch.where(eos, full_eos, tgt)
        return tgt
